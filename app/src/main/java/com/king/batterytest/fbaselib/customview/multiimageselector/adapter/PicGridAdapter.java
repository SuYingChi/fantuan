...
public class PicGridAdapter extends BaseAdapter {
	...
	public PicGridAdapter(Context context, boolean showCamera, int column) {
		mContext = context;
		...
		void bindData(final Image data) {
			...
			if (showSelectIndicator) {
				...
				if (mSelectedImages.contains(data)) {
					...
					indicator.setImageResource(R.drawable.lib_btn_unselected);
			...
			if (imageFile.exists()) {
				...
				Glide.with(mContext).load(imageFile)
                        .apply(new RequestOptions()
                                .placeholder(R.drawable.lib_default_error)
                                .override(mGridWidth, mGridWidth).centerCrop())
						.into(image);

			} else {
				image.setImageResource(R.drawable.lib_default_error);
			}
		}
	}
